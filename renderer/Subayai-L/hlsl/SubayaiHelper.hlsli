...
// ------------------------------------------------
// SS-VPL (Screen-Space Virtual Point Light)
// ------------------------------------------------
struct VPL {
    float3 position; // 光源のワールド座標
    float3 normal;   // 光源の法線
    float3 emission; // 発光カラー
};

// 最大サンプリング点数
#define MAX_VPL_COUNT 		1048576		// 2880x2800=829万ピクセルを想定したとき, 100万ぐらい光源ピクセルとしてとって置ける
#define RAY_SAMPLES_DEF 	8			// 1光源ピクセル当たり8本. ざっくり実験したところで24sampleぐらいで収束しそうなので, 8 x 24 = 192本/ピクセル

// レイトレパスで使うVPLサンプリング用の乱数ヘルパー(てきとう)
float VPL_Rand(uint2 pixel, float time, int sampleIndex) {
    uint state = pixel.x * 1973 + pixel.y * 9277 + (uint)(time * 1000.0) * 26699 + sampleIndex * 12345;
    state = (state ^ 61) ^ (state >> 16);
    state *= 9;
    state = state ^ (state >> 4);
    state *= 0x27d4eb2d;
    state = state ^ (state >> 15);
    return frac((float)state / 4294967296.0);
}
